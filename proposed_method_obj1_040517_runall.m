...
% savefile = ['left_right_lens_boundary_xy_',soft_ndiris_training{k,1},'.mat'];
% [stat,mess]=fileattrib(savefile);

% if exist(strcat('left_right_lens_boundary_xy_',soft_ndiris_training{k,1},'.mat'), 'file') == 0
if exist(strcat(soft_ndiris_training{k,1},'-lens_segmented.jpg'), 'file') == 0
...
% left_lens_boundary_x_single = single(left_lens_boundary_x);
% left_lens_boundary_y_single = single(left_lens_boundary_y);
% right_lens_boundary_x_single = single(right_lens_boundary_x);
% right_lens_boundary_y_single = single(right_lens_boundary_y);

% [f_left_lens_boundary_x_single,sift_left_lens_boundary_x_single] = vl_sift(left_lens_boundary_x_single);
% [f_left_lens_boundary_y_single,sift_left_lens_boundary_y_single] = vl_sift(left_lens_boundary_y_single);
% [f_right_lens_boundary_x_single,sift_right_lens_boundary_x_single] = vl_sift(right_lens_boundary_x_single);
% [f_right_lens_boundary_y_single,sift_right_lens_boundary_y_single] = vl_sift(right_lens_boundary_y_single);

% left_lens_boundary_x=imsmooth(left_lens_boundary_x,.1);

% left_lens_boundary_x_sift=left_lens_boundary_x-min(left_lens_boundary_x(:)) ;
% left_lens_boundary_x_sift=left_lens_boundary_x/max(left_lens_boundary_x(:)) ;
% 
% [hog_features_left, hog_vis_left] = extractHOGFeatures(left_lens_boundary_x,'CellSize',[4 4]);

% S=3;
% 
% [frames1,descr1,gss1,dogss1] = sift( left_lens_boundary_x_sift, 'Verbosity', 1, 'Threshold', ...
%                                      0.005, 'NumLevels', S ) ;

% eval(sprintf('left_lens_boundary_xy_%s = vertcat(sift_left_lens_boundary_x_single,sift_left_lens_boundary_y_single);', soft_ndiris_training{1,1}));
% eval(sprintf('right_lens_boundary_xy_%s = vertcat(sift_right_lens_boundary_x_single,sift_right_lens_boundary_y_single);', soft_ndiris_training{1,1}));

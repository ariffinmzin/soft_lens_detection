...
% all the features are from testing dataset (nd_data_testing_LG4000)
...
% run = 0; % 0 for soft
run = 1; %  1 for no - run on 20/04/18
...
%     try
    ...
    if (exist(strcat(soft_ndiris_testing{k,1},'-lens_segmented.jpg'), 'file') == 0 && strcmp(nd_data_testing_LG4000{j,1},'06117d121')==0 && strcmp(nd_data_testing_LG4000{j,1},'06117d91')==0) % if segmented jpg files does not exist, run createiristemplate

display(soft_ndiris_testing{k,1});
[template, mask, xl_t, yla_t, yla_b, yl_b] = createiristemplate(strcat(soft_ndiris_testing{k,1},'.tiff'), soft_ndiris_testing(k,2), soft_ndiris_testing(k,3), soft_ndiris_testing(k,4), soft_ndiris_testing(k,5), soft_ndiris_testing(k,6), soft_ndiris_testing(k,7));

w = cd;
cd(DIAGPATH);

Inoise = imread(strcat(soft_ndiris_testing{k,1},'.tiff-noise.jpg'));
...
ci = [soft_ndiris_testing{k,6},soft_ndiris_testing{k,5},soft_ndiris_testing{k,7}];     % center and radius of circle ([c_row, c_col, r])
...
ci2 = [soft_ndiris_testing{k,6},soft_ndiris_testing{k,5},soft_ndiris_testing{k,7}+40];     % center and radius of circle ([c_row, c_col, r])
...
[x1, y2] = getpeaks2(peaks,Inoise,yla_t,yla_b,x1,soft_ndiris_testing{k,1});
...
eval(sprintf('Inoise_segmented_left_%s = Inoise(left_lens_boundary_y,left_lens_boundary_x);',soft_ndiris_testing{k,1}));
eval(sprintf('Inoise_segmented_left_%s = double(Inoise_segmented_left_%s);',soft_ndiris_testing{k,1},soft_ndiris_testing{k,1}));
eval(sprintf('Inoise_segmented_left_%s(find(Inoise_segmented_left_%s==0)) = NaN;',soft_ndiris_testing{k,1},soft_ndiris_testing{k,1}));

eval(sprintf('imwrite(Inoise_segmented_left_%s,''%s_left_lens_region.jpg'')',soft_ndiris_testing{k,1},soft_ndiris_testing{k,1}));

imds = imageDatastore(strcat(soft_ndiris_testing{k,1},'_left_lens_region.jpg')); % lagi elok kalau dpt baca direct Inoise segmented left
bag = bagOfFeatures(imds,'GridStep',[1 1]);
img = readimage(imds, 1);

% bag of words;
eval(sprintf('featureVector_left_%s = encode(bag, img);',soft_ndiris_testing{k,1}));

save(strcat('featureVector_left_',soft_ndiris_testing{k,1},'.mat'),strcat('featureVector_left_',soft_ndiris_testing{k,1}));

% right contact lens

eval(sprintf('Inoise_segmented_right_%s = Inoise(right_lens_boundary_y,right_lens_boundary_x);',soft_ndiris_testing{k,1}));
eval(sprintf('Inoise_segmented_right_%s = double(Inoise_segmented_right_%s);',soft_ndiris_testing{k,1},soft_ndiris_testing{k,1}));
eval(sprintf('Inoise_segmented_right_%s(find(Inoise_segmented_right_%s==0)) = NaN;',soft_ndiris_testing{k,1},soft_ndiris_testing{k,1}));

eval(sprintf('imwrite(Inoise_segmented_left_%s,''%s_right_lens_region.jpg'')',soft_ndiris_testing{k,1},soft_ndiris_testing{k,1}));

imds = imageDatastore(strcat(soft_ndiris_testing{k,1},'_right_lens_region.jpg'));
bag = bagOfFeatures(imds,'GridStep',[1 1]);
img = readimage(imds, 1);

% bag of words
eval(sprintf('featureVector_right_%s = encode(bag, img);',soft_ndiris_testing{k,1}));

save(strcat('featureVector_right_',soft_ndiris_testing{k,1},'.mat'),strcat('featureVector_right_',soft_ndiris_testing{k,1}));
...
%         try
            ...
            if (strcmp(nd_data_testing_LG4000{j,6},'No')==1)
                ...
                if exist(strcat(no_ndiris_testing{k,1},'-lens_segmented.jpg'), 'file') == 0 % if segmented jpg files does not exist, run createiristemplate
...
%                     [template, mask, xl_t, yla_t, yla_b, yl_b] = createiristemplate(strcat('C:\Users\Arif\Documents\MATLAB\NDIRIS\LG4000\images\',no_ndiris_training{k,1},'.tiff'), no_ndiris_training(k,2), no_ndiris_training(k,3), no_ndiris_training(k,4), no_ndiris_training(k,5), no_ndiris_training(k,6), no_ndiris_training(k,7));
                    [template, mask, xl_t, yla_t, yla_b, yl_b] = createiristemplate(strcat(no_ndiris_testing{k,1},'.tiff'), no_ndiris_testing(k,2), no_ndiris_testing(k,3), no_ndiris_testing(k,4), no_ndiris_testing(k,5), no_ndiris_testing(k,6), no_ndiris_testing(k,7));

                    w = cd;
                    cd(DIAGPATH);

                    Inoise = imread(strcat(no_ndiris_testing{k,1},'.tiff-noise.jpg'));
                    ...
                    imageSize = size(Inoise);
                    % ci = [soft_ndiris_training{1,6},soft_ndiris_training{1,5},soft_ndiris_training{1,7}];     % center and radius of circle ([c_row, c_col, r])
                    ci = [no_ndiris_testing{k,6},no_ndiris_testing{k,5},no_ndiris_testing{k,7}];     % center and radius of circle ([c_row, c_col, r])
                    [xx,yy] = ndgrid((1:imageSize(1))-ci(1),(1:imageSize(2))-ci(2));
                    mask = logical((xx.^2 + yy.^2)>ci(3)^2);
                    croppedImage = logical(false(size(Inoise)));
                    croppedImage(:,:,1) = lines(:,:,1).*mask;
                    ...
                    imageSize = size(Inoise);
                    % ci2 = [soft_ndiris_training{1,6},soft_ndiris_training{1,5},soft_ndiris_training{1,7}+40];     % center and radius of circle ([c_row, c_col, r])
                    ci2 = [no_ndiris_testing{k,6},no_ndiris_testing{k,5},no_ndiris_testing{k,7}+40];     % center and radius of circle ([c_row, c_col, r])
                    [xx,yy] = ndgrid((1:imageSize(1))-ci2(1),(1:imageSize(2))-ci2(2));
                    mask2 = logical((xx.^2 + yy.^2)<ci2(3)^2);
                    % mask2 = uint8(mask<ci2(3)^2);
                    croppedImage2 = logical(false(size(Inoise)));
                    ...
                    % [x1, y2] = getpeaks2(peaks,Inoise,yla_t,yla_b,x1,soft_ndiris_training{1,1});
                    [x1, y2] = getpeaks2(peaks,Inoise,yla_t,yla_b,x1,no_ndiris_testing{k,1});
...
%                     eval(sprintf('Inoise_segmented_left_%s = Inoise(left_lens_boundary_x,left_lens_boundary_y);',no_ndiris_training{k,1})); % Inoise(index,index)
                    eval(sprintf('Inoise_segmented_left_%s = Inoise(left_lens_boundary_y,left_lens_boundary_x);',no_ndiris_testing{k,1})); % Inoise(index,index)
                    eval(sprintf('Inoise_segmented_left_%s = double(Inoise_segmented_left_%s);',no_ndiris_testing{k,1},no_ndiris_testing{k,1}));
                    eval(sprintf('Inoise_segmented_left_%s(find(Inoise_segmented_left_%s==0)) = NaN;',no_ndiris_testing{k,1},no_ndiris_testing{k,1}));
                    ...
                    eval(sprintf('imwrite(Inoise_segmented_left_%s,''%s_left_lens_region.jpg'')',no_ndiris_testing{k,1},no_ndiris_testing{k,1}));

                    imds = imageDatastore(strcat(no_ndiris_testing{k,1},'_left_lens_region.jpg'));
                    bag = bagOfFeatures(imds,'GridStep',[1 1]);
                    img = readimage(imds, 1);
                    % featureVector = encode(bag, img);
                    eval(sprintf('featureVector_left_%s = encode(bag, img);',no_ndiris_testing{k,1}));

                    save(strcat('featureVector_left_',no_ndiris_testing{k,1},'.mat'),strcat('featureVector_left_',no_ndiris_testing{k,1}));
                    ...
                    try
                    %428489
                    eval(sprintf('Inoise_segmented_right_%s = Inoise(right_lens_boundary_x,right_lens_boundary_y);',no_ndiris_testing{k,1}));
                    eval(sprintf('Inoise_segmented_right_%s = double(Inoise_segmented_right_%s);',no_ndiris_testing{k,1},no_ndiris_testing{k,1}));
                    eval(sprintf('Inoise_segmented_right_%s(find(Inoise_segmented_right_%s==0)) = NaN;',no_ndiris_testing{k,1},no_ndiris_testing{k,1}));

                    eval(sprintf('imwrite(Inoise_segmented_left_%s,''%s_right_lens_region.jpg'')',no_ndiris_testing{k,1},no_ndiris_testing{k,1}));

                    imds = imageDatastore(strcat(no_ndiris_testing{k,1},'_right_lens_region.jpg'));
                    bag = bagOfFeatures(imds,'GridStep',[1 1]);
                    img = readimage(imds, 1);
                    % featureVector = encode(bag, img);
                    eval(sprintf('featureVector_right_%s = encode(bag, img);',no_ndiris_testing{k,1}));

                    save(strcat('featureVector_right_',no_ndiris_testing{k,1},'.mat'),strcat('featureVector_right_',no_ndiris_testing{k,1}));
